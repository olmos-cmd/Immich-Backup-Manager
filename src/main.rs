...
fn unprotect_api_key(encoded: &str) -> Result<String, String> {
    ...
    if ok == 0 {
        return Err("Windows DPAPI could not decrypt the API key for this Windows user.".to_string());
    }
            ...
            .min_width(620.0).min_height(420.0).open(&mut open).show(ctx, |ui| {
                ...
                egui::ScrollArea::vertical().id_salt("info_scroll").scroll_bar_visibility(egui::containers::scroll_area::ScrollBarVisibility::AlwaysVisible).auto_shrink([false,false]).show(ui, |ui| {
                    if en {
                        ...
                        ui.monospace(r"%APPDATA%\Immich_Backup_Manager\settings.json");
                        ui.add_space(6.0);
                        ui.label("If version 1.3.1 previously stored the API key as plain text, the application automatically encrypts and migrates it when the settings are loaded successfully for the first time. The saved key can be removed at any time using the ‘Delete saved API key’ button.");
                        ui.add_space(6.0);
...
fn show_download_popup(&mut self, ctx: &egui::Context) {
    ...
    fn ensure_current_conflict_preview(&mut self, ctx: &egui::Context) {
        ...
        if let Ok(bytes) = fs::read(&local_path) {
            if let Some((texture, dims)) = Self::texture_and_size_from_bytes(ctx, &format!("conf_local_{}", item.job.asset.id), &bytes, true) {
        ...
        if let (Ok(base), Ok(key), Ok(client)) = (self.base_url(), self.ensure_api_key(), self.client()) {
            ...
            if let Ok(resp) = client
                ...
                if let Ok(bytes) = resp.bytes() {
                    if let Some((texture, dims)) = Self::texture_and_size_from_bytes(ctx, &format!("conf_remote_{}", item.job.asset.id), bytes.as_ref(), false) {
    ...
    fn show_conflict_popup(&mut self, ctx: &egui::Context) {
        ...
        if let Some(jobs) = jobs {
            if jobs.is_empty() {
                ...
                self.status = if english { format!("Error: {}", e) } else { format!("Fehler: {}", e) };
            }
        }
